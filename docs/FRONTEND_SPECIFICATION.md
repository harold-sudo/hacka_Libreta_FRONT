...
### Compra Unlock: confirmar cambio de red antes del firmante

El reporte keyPrice BAD_DATA procede de index-CxydwUrT.js; al inspeccionar el sitio durante el diagnóstico servía index-D_6K0mo9.js con Sepolia ya configurada. El usuario indicó que el error ocurre en su deploy actual; no se verificó la red efectiva de su MetaMask. Por tanto no se atribuye el caso exclusivamente a caché.

Se corrigió un riesgo del flujo local: obtenía signer antes de cambiar de red y no comprobaba el resultado de wallet_switchEthereumChain. prepareUnlockWallet usa eth_chainId directo antes/después del cambio, exige la red del Lock y crea un BrowserProvider nuevo fijado a esa red, con caché desactivada. Solo después se validan red/bytecode y se obtiene signer para keyPrice/purchase. Si se agrega una red se solicita explícitamente el cambio después. Una wallet que permanece en otra cadena se rechaza antes de consultar o enviar. Un cambio posterior no se sigue silenciosamente.

El argumento bytes[] vacío por destinatario ahora es ['0x'] tanto en estimateGas como en purchase. Pruebas: nueve casos combinados en tests/unlock-wallet.test.mjs y tests/unlock-deployment.test.mjs; incluyen cambio HSK/Sepolia, keyPrice con proveedor ethers real y wallet simulada, rechazo, cambio sin efecto, agregar/cambiar red y cambio posterior a HSK. Lint y build con variables Sepolia correctos. No se compró ninguna Key ni se envió una transacción; falta deploy y prueba con la wallet del usuario.
